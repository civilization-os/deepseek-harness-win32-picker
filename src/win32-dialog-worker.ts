 ...
 * the IPC channel. Spawned as a child process (not a worker thread); the COM
 * conversation owns the dialog to the foreground browser window so Windows
 * keeps it above that window. Protocol: `{kind:'showing',threadId}` right
...
const post = (message: Win32DialogWorkerMessage): void => {
  ...
  /* v8 ignore next 3 -- disconnect needs a live IPC channel that unit tests must not sever. */
...
/* v8 ignore next 3 -- the handler exits(0), which would terminate a unit test process. */
