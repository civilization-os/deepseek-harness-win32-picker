...
export interface Win32FolderDialog {
  ...
  /**
   * `IModalWindow::Show`; blocks the calling thread
   * until the user selects or dismisses.
   * @param owner - foreground top-level window that keeps the dialog in front,
   * or null when Windows reports no foreground window.
...
export interface Win32DialogBindings {
  ...
  /**
   * `GetForegroundWindow` immediately before `Show`.
   * @returns the current foreground window handle, or null when none exists.
   */
